...
このリポジトリには実装コード、環境変数(.env)、Bubble/Make/LINE等の設定値、APIキー、Webhook URLといった機密情報は一切含めません。親リポジトリを含むGitHub repositoryはPublicであり、秘密値はGitHub Actions Secrets等の承認済みsecret storeで管理します。
...
本体(Public)リポジトリでは、このリポジトリが `docs/` としてgit submoduleでマウントされています。設計書の実体はこのリポジトリのみに存在し、本体側には複製を持ちません。
...
- **設計書・ADR・Decision・検証記録は、この公開docsリポジトリへ置く。** 親リポジトリだけに設計Decisionを置かない。
- **親リポジトリもPublicである。** 実装コード・画像資産・秘密値を含まない設定を置き、Secrets・token・個人情報・商用機密は両リポジトリへ含めない。
- **AIレビューを依頼する際は、GitHubのtreeナビゲーションに依存せず、対象ファイルの直接URL(blobリンク)を渡す。** 上記「ドキュメント一覧」「ADR一覧」の直接リンクを利用すること。
- **作業開始時に両リポジトリの現在のvisibilityをGitHub一次情報で確認する。** アクセスできない場合はprivateと推測せず、認証・ネットワーク・repository名変更・visibility変更を区別して報告する。
