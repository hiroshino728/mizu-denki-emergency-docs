...
文書やコミットメッセージを読む際、以下の4種類を区別してください。混同すると、まだ検証していない仮説やAIによる疑似検証を確定事項として扱ってしまうリスクがあります。
...
- **Synthetic Insight（AIによる疑似検証）**: AIエージェントによる疑似ペルソナレビュー・シミュレーション等の結果。表現の下書き・仮説出しには使えるが、Assumption（仮説）の裏付けやExternal Evidence（実際の顧客・加盟店の反応）の代替として扱わない。引用する場合は必ず「Synthetic Insight」であることを明記する。
