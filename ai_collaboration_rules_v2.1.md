...
## 10. 外部への送信・公開を伴うアクションの事前検証義務

背景: 2026-08-22、加盟店候補6社へのメール送信において、Gmail連携ツールに送信元(From)を指定するパラメータが存在しないため、屋号アドレス(info@mizudenki-kyukyu.jp)の設定が完了していたにもかかわらず、実際の送信は篠さんの個人アドレスから行われた。「設定完了の確認」と「実際の送信メカニズムの検証」を別工程として扱わず、後者を省略したまま外部の実在する相手への本番実行を行ったことが原因である。

ルール: AIエージェント(Claude Code / Codex / Claude chatを問わない)が、以下のいずれかに該当するアクションを、実在する外部の相手に対して初めて実行する前には、必ず自分が管理する宛先(担当者自身のメールアドレス、テスト用アカウント等)への試験実行を行い、実際の出力(送信元ヘッダー、表示内容、公開範囲等)を確認すること。

- 外部の第三者へのメール送信・メッセージ送信
- Webサイト・公開リポジトリ等への外部公開
- その他、実行結果が第三者に到達し、取り消しが困難なアクション

「設定画面上で完了と表示されている」ことは、動作確認の代替にならない。設定完了の確認と、実機での動作検証は、別のチェック項目として扱うこと。

試験実行を行えない技術的制約がある場合、当該アクションの外部実行自体を保留し、その制約を明示した上で人間(篠さん)へエスカレーションする。制約を承知の上で妥協的に実行してはならない。

記録日: 2026-08-22(Issue #23関連インシデントを踏まえて追加)# AI協働運用ルール v2.1
...
Actions権限・Secrets・Production環境の変更を伴うPRは、実装したエージェント自身による`review:self`のみでは独立レビュー要件を満たさない。別モデルまたは別エージェントによるレビューが完了するまで、本番有効化(Tier 1自動マージのlive化、Bubble Production反映等)を行わない。`review:self`はセルフチェックとして記録できるが、独立レビューとして数えない。

低リスクな変更(docs更新、ドラフト段階の設計、テスト)まで別AI待ちで止める必要はない。このルールはIssue #34で発生した、実装者自身の`review:self`とstaleな巻き戻しPRの有人検知を踏まえた最小限の再発防止であり、AI協働運用基盤の新規機能拡張ではない。
