...
以下のいずれかに該当する変更は、Tier 1/Tier 2いずれの判定結果によらず、変更を行った本人(同一AIエージェント・同一セッション)以外――篠さん、またはそれ以外のAIエージェント・別セッション――による独立レビューを経てからマージする。担当者自身によるセルフレビュー(`review:self`)のみでのマージは、以下に該当する変更には適用しない。

- docs submoduleのgitlink参照を、現在の`main`から辿れない、または巻き戻す方向のcommitへ向けるPR(stale・別系統のcommitを含む)
- 自動マージworkflow(`.github/workflows/`配下)自体の判定ロジック・fail-closed条件を変更するPR
- 本節(第8節)のTier 1/Tier 2判定条件そのものを変更するPR

この節は新規のGovernance拡張ではなく、親リポジトリ`hiroshino728/water-denki-emergency-mvp`のIssue #34で実際に発生したインシデントへの穴埋めである。Stage 2初回有人監視で、staleかつ巻き戻し方向のPR #32(`ef5661b`から`dc7d783`への巻き戻し、CONFLICTING)を検出した。当時はマージ前に人間の監視によって検出できたが、これを検出するステップ自体は運用ルール上の必須要件として明文化されていなかった。今後、上記に該当する変更は、独立レビューを経る前提を本節で明文化する。
...
- 2026-08-22: 第8節に「高リスク変更の独立レビュー」を追加。Issue #34で発生したstale/巻き戻しPR(#32)インシデントへの穴埋めであり、新規のGovernance拡張ではない。
