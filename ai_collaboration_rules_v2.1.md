...
現時点で、GitHub Issue運用ルール(本ドキュメント)・親リポジトリ`AGENTS.md`・自動化ワークフロー(PRマージのTier分類、AI discussion pipeline等)を含む「AI協働運用基盤」そのもののさらなる拡張は凍結する。

- **凍結の対象:** 新規の自動化ワークフローの追加、Tier 1(自動マージ)対象範囲の拡大、新たな運用ルール・ラベル体系・Gate判定基準の追加など、AI協働運用基盤そのものを拡張する変更
- **凍結の対象外:** 既存ルールの範囲内で行う通常の開発・設計作業――GATE-A〜Eの事業検証実行支援(ヒアリング文面下書き、記録整理等)、設計書更新、バグ修正、既存ルールの誤字・矛盾修正等
- **凍結解除条件:** CEO(篠)の明示的なDecisionによる。「GATE-A〜Dが進捗するまで」のような、達成が曖昧または段階的に解釈しうる条件では解除しない。凍結解除は、篠さんが明示的にDecisionとして示した時点でのみ成立する。

この凍結が適用されている間、AIエージェントは新たな自動化・運用ルールの追加を自発的に提案しない。事業検証(GATE-A〜E)の実行支援は、この凍結の対象外として引き続き行う。

---

## 更新履歴

- 2026-08-22: 第9節「AI協働運用基盤・自動化拡張の凍結」を追加。凍結解除条件をCEO(篠)の明示的なDecisionに固定した。
